        ...
        <div class="card shadow-sm rounded-4 border-0">
            <div class="card-body p-4 p-md-5" style="background: linear-gradient(180deg, rgba(239,230,218,0.95), rgba(255,255,255,0.95));">
                <div class="d-flex justify-content-between align-items-center mb-4">
                    <div>
                        <h1 class="h4 mb-1">Crear Producto</h1>
                        <p class="text-muted mb-0">Agrega un nuevo producto al catálogo</p>
                    </div>
                    <span class="badge rounded-pill" style="background: var(--brown-300); color: var(--brown-900);">Nuevo</span>
                </div>
                ...
                <form action="{{ route('products.store') }}" method="POST" enctype="multipart/form-data" novalidate>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" name="name" value="{{ old('name') }}" class="form-control @error('name') is-invalid @enderror rounded-pill border-0 shadow-sm" placeholder="Nombre del producto">
                        @error('name')
                    ...
                    <div class="row g-3">
                        <div class="col-md-6">
                            ...
                            <input type="number" step="0.01" name="price" value="{{ old('price') }}" class="form-control @error('price') is-invalid @enderror rounded-pill border-0 shadow-sm" placeholder="0.00">
                            @error('price')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        <div class="col-md-6">
                            ...
                            <input type="number" name="stock" value="{{ old('stock') }}" class="form-control @error('stock') is-invalid @enderror rounded-pill border-0 shadow-sm" placeholder="Cantidad disponible">
                            @error('stock')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                    ...
                    <div class="d-flex justify-content-end mt-4">
                        <button type="submit" class="btn btn-primary-custom px-4 py-2">Guardar producto</button>
                    </div>
                </form>
            </div>
        </div>
